...
function Home({navigation, workouts, handle, sessionTeaser}) {
  ...
  const [nextWorkoutKey, setNextWorkoutKey] = useState(
    sessionTeaser.workoutName === 'Texas Method A'
      ? workouts[1].key
      : workouts[0].key,
  );
    ...
    <View style={[styles.container]} testID="home-root">
      <ActionCard
        actionComponent={
          <ActionButton
            text="List"
            label="List Sessions"
            action={() => navigation.navigate('SessionsList')}
          />
        }>
      ...
      <ActionCard
        actionComponent={
          <ActionButton
            text="Go"
            label="Go to next workout"
            action={() => {
              handle.nextWorkoutInteraction(nextWorkoutKey);
              navigation.navigate('Workout', {
                action: 'go',
                title: workouts.find(el => el.key === nextWorkoutKey).name,
              });
            }}
          />
        }>
      ...
      <ActionCard
        actionComponent={
          <ActionButton
            text="Edit"
            label="Edit Workouts"
            action={() => navigation.navigate('WorkoutsList')}
          />
        }>
      ...
      <ActionCard
        actionComponent={
          <ActionButton
            ...
            action={() => navigation.navigate('ExercisesList')}
          />
        }>
