...
describe('<AddExerciseSet />', () => {
  ...
  it('renders default values with toAdd', () => {
    ...
    expect(renderer.getByTestId('weight-picker').props.selectedValue).toBe(
      '100',
    );
  ...
  it('renders passed in item with toAdd false', () => {
    ...
    renderer = render(
      <AddExerciseSet item={item} updater={updater} toAdd={false} />,
    );
    instance = renderer.getByTestId('add-exercise-set-root');
    ...
    expect(updater).toHaveBeenLastCalledWith(
      {reps: '5', weight: '145'},
      1,
      'REMOVE_SET',
    );
