...
beforeEach(() => {
  renderer = render(
    <Session session={getSession()} navigation={navigation} handle={handle} />,
  );
...
describe('<Session />', () => {
  ...
  it('moves set list back and forward when progress buttons are pressed and complete acts different from skip', () => {
    ...
    expect(renderer.queryByTestId('sets-reps-table-sets').props.children).toBe(
      '1/6',
    );
    expect(
      renderer.queryByTestId('sets-reps-table-sets').props.children,
    ).not.toBe('2/6');
    expect(
      renderer.queryByTestId('sets-reps-table-completed').props.children,
    ).toBe('false');
    expect(renderer.queryByTestId('sets-reps-table-reps').props.children).toBe(
      '5',
    );
    expect(
      renderer.queryByTestId('sets-reps-table-weight').props.children,
    ).toBe('45 lbs');
  ...
  it('Weight plates string displays correctly as sets move forward', () => {
    ...
    expect(
      renderer.queryByTestId('time-plates-table-plates').props.children,
    ).toBe(' None');
  ...
  it('Timer progresses and set timer resets when new set and exercise proceed', () => {
    jest.useFakeTimers();
    expect(renderer.queryByTestId('time-plates-table-set').props.children).toBe(
      0,
    );
    expect(
      renderer.queryByTestId('time-plates-table-session').props.children,
    ).toBe(0);
  ...
  it('Resumes session at current exercise, set, and duration state as user left', () => {
    ...
    renderer = render(
      <Session session={session} navigation={navigation} handle={handle} />,
    );

    expect(
      renderer.queryByTestId('time-plates-table-session').props.children,
    ).toBe(50);
    expect(renderer.queryByTestId('current-exercise').props.children).toBe(
      'Squats',
    );
    expect(renderer.queryByTestId('sets-reps-table-sets').props.children).toBe(
      '3/5',
