    ...
    if (actionType === 'go') {
      add = (
        <ActionButton
          ...
          key="add"
          text="Go"
          action={() => update('CREATE_SESSION')}
        />
      );
    } else {
      add = (
        <ActionButton
          label={`Update ${workout.name} workout`}
          key="add"
          text="Save"
          action={() => update('UPDATE_WORKOUT')}
        />
      );
    }
  ...
  return (
    <View style={{padding: 10, flex: 1}} testID="workout-root">
      <View style={{flex: 2}}>
        ...
        {/*</View>*/}
        <View style={{flex: 1, flexDirection: 'row', alignItems: 'center'}}>
          <Text style={[styles.bold, styles.mediumTextInputFont, {flex: 1}]}>
            Info
          </Text>
          <TextInput
            style={[styles.editText, styles.smallTextInputFont, {flex: 4}]}
        ...
        <View
          ...
          <Picker
            ...
            {transformers.map(tr => (
              <Picker.Item key={tr.key} label={tr.label} value={tr.key} />
            ))}
          </Picker>
        </View>
      </View>
      <View style={{flex: 8}}>
        <WorkoutExerciseListProvider
          workout={workout}
