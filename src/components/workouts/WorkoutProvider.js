...
function getWorkout(workouts, activeWorkoutKey, navigation, transformer) {
  ...
  return transformer.key !== 'none'
    ? transform(workouts[activeWorkoutKey], transformer)
    : workouts[activeWorkoutKey];
}
...
const mapStateToProps = (state, otherProps) => {
  return {
    workout: getWorkout(
      ...
      state.transformers[state.activeTransformerKey],
    ),
    transformers: Object.keys(state.transformers).map(
      key => state.transformers[key],
    ),
    actionType:
      otherProps.navigation.getParam('action', '') === 'go' ? 'go' : 'save',
  };
};
