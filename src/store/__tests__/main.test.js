...
describe('Exercises Reducer', () => {
  ...
  it('CREATE_EXERCISE', () => {
    ...
    expect(Object.keys(reducer.exercises).length).toBeGreaterThan(Object.keys(data.exercises).length);
  });
  ...
  it('UPDATE_EXERCISE', () => {
    ...
    let reducer = store(undefined, {type: WorkoutActions.UPDATE_EXERCISE, exercise: newExercise});
    expect(reducer.exercises[squatsKey].name).toBe('Not Squats');
    expect(reducer.exercises[squatsKey].sets[0]).toEqual({reps: 7, weight: 260.0});
  });

  it('SET_ACTIVE_EXERCISE', () => {
    const reducer = store(undefined, {type: WorkoutActions.SET_ACTIVE_EXERCISE, key: squatsKey});
  ...
  it('UPDATE_SET', () => {
    let newData = Object.assign({}, data);
    newData.activeExerciseKey = squatsKey;
    const reducer = store(newData, {type: WorkoutActions.UPDATE_SET, index: 0, set: {reps: 7, weight: 260.0}});
    expect(reducer.exercises[squatsKey].sets[0]).toEqual({reps: 7, weight: 260.0});
    expect(reducer.theThing).not.toEqual(data.theThing);
  });
...
describe('Sessions Reducer', () => {
  ...
  it('SET_ACTIVE_SESSION', () => {
    const reducer = store(undefined, {type: WorkoutActions.SET_ACTIVE_SESSION, key: firstSession});
  ...
  it('FINISH_SESSION', () => {
    ...
    const reducer = store(undefined, {type: WorkoutActions.FINISH_SESSION, session: aSesh});
